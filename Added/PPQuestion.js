...
var PPQuestionState = {
  ...
  create: function () {
    ...
    // Randomize options (unchanged)
    if (PPGame.optionOrder.length == 0) {
      var randomOptions = [];
      for (var i = 0; i < options.length; ++i) {
        randomOptions.push({ id: i, obj: options[i] });
      }
    ...
    // ----- Choice Buttons (with keyboard support) -----
    var buttonWidth = WIDTH * (options.length == 3 ? 0.33 : 0.42);
    this.optionButtons = []; // collect them in visual order

    for (var i = 0; i < PPGame.optionOrder.length; ++i) {
      // Use a local scope per iteration for safety
      var idx = i;

      var onClick = function (ref) {
        // ref is expected to be the button; fallback to `this` if needed
        var btn = ref && ref.optionIndex !== undefined ? ref : this;
        PPGame.chosenOptionId = btn.optionIndex;
      ...
      var optionButton = this.add.button(
        xOffset,
        0.68 * HEIGHT,
        PPGame.optionOrder[idx].obj.name,
        onClick,
        this,
        0,
        0,
      ...
      optionButton.optionIndex = PPGame.optionOrder[idx].id;

      // Gentle idle pulse (unchanged)
      this.add
      ...
      // Collect for keyboard navigation in the VISUAL order
      this.optionButtons.push(optionButton);
    }

    // Enable keyboard navigation for the option buttons (horizontal)
    // Helper is defined in Main.js
    this.kbNav = enableKeyboardForButtons(this, this.optionButtons /*, { vertical: true }*/);
  ...
  // Clean up keyboard handlers when leaving the state
  shutdown: function () {
